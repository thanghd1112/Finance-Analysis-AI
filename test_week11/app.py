...
def display_financial_report(ticker):
    ...
    if report_html:
        # Read CSS and JS content to embed directly into HTML
        try:
            ...
            # Save HTML file with embedded CSS and JS
...
if analysis_type == "Phân tích Cơ bản" and (analyze_button or st.session_state.analyzed_data is not None):
    ...
    if financial_data is None:
        ...
        with tab2:
            # Phân tích kỹ thuật
            tech_fig_base64 = agent.plot_technical_indicators(ticker)
            if tech_fig_base64:
                st.image(f"data:image/png;base64,{tech_fig_base64}", use_column_width=True)
            else:
