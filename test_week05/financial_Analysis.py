...
class AIAgent:
    ...
    def predict_future_price(self, ticker, days=30, mode='add'):
        """Predict stock price using Prophet model with add/mul modes"""
        if ticker not in self.data:
            print(f"Không tìm thấy dữ liệu cho {ticker}. Vui lòng lấy dữ liệu trước.")
            return None, None
        ...
        try:
            ...
            # Sử dụng StockPredictor để dự đoán với chế độ được chọn
            plt_predict, future_prices = self.predictor.predict(price_data, days=days, mode=mode)
            
            if plt_predict is not None:
                plt_predict.suptitle(f'Dự đoán giá cổ phiếu {ticker} - Mô hình {mode.upper()}')
