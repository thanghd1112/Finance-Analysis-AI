    ...
    if not ticker:
        ...
        with st.spinner(f"Đang phân tích {ticker}..."):
            ...
            if financial_data is None:
                ...
                st.session_state.predictions_add = None
                st.session_state.predictions_mul = None
                ...
                with tabs[1]:
                    st.subheader(f"Dự đoán giá - Mô hình {st.session_state.prediction_mode.upper()}")


                    
                    # Nút chuyển đổi chế độ dự đoán
                    col1, col2 = st.columns([0.9, 0.1])
                    with col2:
                        toggle_button = st.button("🔄", key="toggle_prediction_mode")
                        if toggle_button:
                    ...
                    if st.session_state.predictions_add is None:
                        st.session_state.predictions_add, _ = agent.predict_future_price(ticker, days=prediction_days, mode='add')
                        if st.session_state.predictions_add is not None:
                            st.pyplot(predictions_add) 
                    if st.session_state.predictions_mul is None:    
                        st.session_state.predictions_mul, _ = agent.predict_future_price(ticker, days=prediction_days, mode='mul')
                    
                    # Tạo layout so sánh hai mô hình
                    expand_prediction = st.checkbox("So sánh 2 mô hình")
                    
                    if expand_prediction:
                        # Layout so sánh hai mô hình
                        col_add, col_mul = st.columns(2)
                        
                        with col_add:
                            st.subheader("Mô hình Cộng (Additive)")
                            if st.session_state.predictions_add is not None:
                                st.pyplot(st.session_state.predictions_add)
                        
                        with col_mul:
                            st.subheader("Mô hình Nhân (Multiplicative)")
                            if st.session_state.predictions_mul is not None:
                                st.pyplot(st.session_state.predictions_mul)
                        
                        # Nhận xét so sánh hai mô hình
                        st.markdown("### Nhận xét so sánh mô hình")
                        st.markdown("""
                        - **Mô hình Cộng (Additive)**: Các xu hướng và mùa vụ được cộng dồn, phù hợp với các xu hướng tuyến tính.
                        - **Mô hình Nhân (Multiplicative)**: Các xu hướng và mùa vụ được nhân với nhau, phù hợp với các xu hướng phi tuyến.
                        - So sánh hai mô hình để chọn mô hình phù hợp nhất với dữ liệu của bạn.
                        """)
                    else:
                        # Mặc định hiển thị mô hình được chọn
                        prediction_fig = st.session_state.predictions_add if st.session_state.prediction_mode == 'add' else st.session_state.predictions_mul
