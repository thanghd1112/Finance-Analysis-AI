...
class StockPredictor:
    ...
    def predict(self, price_data, days=30, mode='additive'):
        """
        ...
        mode: Chế độ mô hình ('additive' hoặc 'multiplicative')
        ...
        try:
            ...
            model = Prophet(
                ...
                interval_width=0.95,
                seasonality_mode=mode  # Thêm chế độ mùa vụ
            )
            ...
            # Vẽ dự đoán
            plt.plot(future_predictions.index, future_predictions['yhat'], 'r--', label=f'Dự đoán ({mode})')
            ...
            plt.title(f'Dự đoán mô hình {mode.capitalize()}')
            plt.grid(True)
            plt.legend()
            plt.tight_layout()
