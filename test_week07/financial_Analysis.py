...
class AIAgent:
    ...
    def save_report_as_pdf(self, ticker, report):        
        pdf_filename = f"reports/{ticker}_report.pdf"
        pdfkit.from_string(report, pdf_filename)
        return pdf_filename
    ...
    def predict_future_price(self, ticker, days=30, multiplicative=False):
        """
        Dự đoán giá cổ phiếu sử dụng mô hình Prophet với điều chỉnh
        khoảng tin cậy phù hợp với từng mô hình
        """
        if ticker not in self.data:
            print(f"Không tìm thấy dữ liệu cho {ticker}. Vui lòng lấy dữ liệu trước.")
            return None, None
        ...
        try:
            ...
            # Sử dụng StockPredictor để dự đoán với khoảng tin cậy được điều chỉnh
            ...
            if plt_predict is not None:
                ...
                # Chú thích đã được thêm trong phương thức predict của StockPredictor
    ...
    def answer_question(self, ticker, question, financial_data=None):
        """
        Trả lời câu hỏi của người dùng dựa trên dữ liệu tài chính
        ...
        # Câu hỏi về biến động giá
        if "tăng" in question_lower or "giảm" in question_lower or "biến động" in question_lower:
            if 'price_history' in financial_data and not financial_data['price_history'].empty:
        ...
        # Câu hỏi về khối lượng giao dịch
        if "khối lượng" in question_lower or "volume" in question_lower:
            if 'price_history' in financial_data and not financial_data['price_history'].empty:
                latest_volume = financial_data['price_history']['Volume'].iloc[-1]
                return f"Khối lượng giao dịch gần nhất của {ticker} là {latest_volume:,.0f} cổ phiếu."
        
        # Câu hỏi về giá cao nhất/thấp nhất trong một khoảng thời gian
        if "cao nhất" in question_lower or "thấp nhất" in question_lower:
            if 'price_history' in financial_data and not financial_data['price_history'].empty:
                highest_price = financial_data['price_history']['High'].max()
                highest_date = financial_data['price_history']['High'].idxmax()
                
                lowest_price = financial_data['price_history']['Low'].min()
                lowest_date = financial_data['price_history']['Low'].idxmin()
                
                if "cao nhất" in question_lower:
                    return f"Giá cao nhất của {ticker} trong khoảng thời gian đã chọn là {highest_price:.2f} vào ngày {highest_date.strftime('%d/%m/%Y')}."
                else:
                    return f"Giá thấp nhất của {ticker} trong khoảng thời gian đã chọn là {lowest_price:.2f} vào ngày {lowest_date.strftime('%d/%m/%Y')}."
        
        # Câu hỏi về thông tin cơ bản của công ty
        if "công ty" in question_lower or "thông tin" in question_lower:
            if 'company_info' in financial_data:
                ...
                if 'longName' in company_info:
                    summary += f"- Tên công ty: {company_info['longName']}\n"
                if 'sector' in company_info:
                    summary += f"- Ngành: {company_info['sector']}\n"
                if 'industry' in company_info:
                    summary += f"- Lĩnh vực: {company_info['industry']}\n"
                if 'website' in company_info:
                    summary += f"- Website: {company_info['website']}\n"
                if 'longBusinessSummary' in company_info:
                    summary += f"- Mô tả: {company_info['longBusinessSummary']}\n"
                    
                return summary
        
        # Trả lời mặc định nếu không xác định được câu hỏi
        return f"Tôi không thể hiểu rõ câu hỏi của bạn. Vui lòng hỏi cụ thể hơn về giá cả, khối lượng giao dịch, hoặc thông tin công ty của {ticker}."