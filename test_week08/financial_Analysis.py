    ...
    def answer_question(self, ticker, question, financial_data=None):
        ...
        if "rsi" in question_lower:
            if 'RSI' in financial_data:
                rsi_value = financial_data['RSI'].iloc[-1]
        ...
        if "bollinger bands" in question_lower or "dải bollinger" in question_lower:
            if 'Upper_Band' in financial_data and 'Lower_Band' in financial_data:
                upper_band = financial_data['Upper_Band'].iloc[-1]
                lower_band = financial_data['Lower_Band'].iloc[-1]
                return f"Dải Bollinger của {ticker}: Upper Band = {upper_band:.2f}, Lower Band = {lower_band:.2f}."

        # 📌 5️⃣ Dự đoán giá tương lai
        if "dự đoán" in question_lower or "xu hướng" in question_lower:
            ...
            if future_prices is not None:
                ...
                return f"Dự đoán xu hướng giá của {ticker} sau {prediction_days} ngày là {predicted_price:.2f}."
        ...
        if "cổ tức" in question_lower or "dividend" in question_lower:
            ...
            return f"Tỷ suất cổ tức của {ticker} là {dividend_yield * 100:.2f}%." if dividend_yield else f"Không có dữ liệu cổ tức cho {ticker}."
        ...
        return f"Không tìm thấy thông tin phù hợp cho câu hỏi về {ticker}."
