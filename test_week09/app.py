...
# CSS đúng và đơn giản để sửa lỗi menu bar
st.markdown("""
<style>
/* Fix menu dropdown */
...
def setup_chat_interface(ticker, agent, financial_data):
    ...
    /* Remove default streamlit input styling */
                ...
                if "```python" in content:
                    ...
                    for i in range(1, len(parts)):
                        if "```" in parts[i]:
                            ...
                            formatted_content += f'<div class="code-block"><pre><code>{code}</code></pre></div>{rest}'
                        else:
                            formatted_content += parts[i]
                    content = formatted_content
                
                st.markdown(
                    f'<div class="chat-message chat-answer">{content}</div>',
                    unsafe_allow_html=True
                )
    ...
    # Add send button
    st.markdown(
        '<button class="send-button" onclick="document.querySelector(\'[data-testid=\\\'stFormSubmitButton\\\']\').click()">▶</button>',
        unsafe_allow_html=True
    )
